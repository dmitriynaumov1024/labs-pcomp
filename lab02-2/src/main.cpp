...
void parallel_mult_thread (matrix_mult_params<double>* params) {
    Matrix<double>* a = params->a;
    Matrix<double>* b = params->b;
    Matrix<double>* c = params->c;
    int cols = c->cols, 
...
Matrix<double>* parallel_mult (Matrix<double>* a, Matrix<double>* b, int num_threads) {
    ...
    for (int i=0; i<num_threads; i++) {
        ...
        threads[i] = thread(parallel_mult_thread, p);
    }
    for (int i=0; i<num_threads; i++) {
...
void read_thread (void* arg, Matrix<double>** result) {
    ...
    *result = new Matrix<double>(rows, cols);
    if (rows == 0 || cols == 0) {
        fclose(source);
        return;
    }
...
Matrix<double>** read (char** filenames, int count, int num_threads) {
    ...
    for (int i=0; i<count; i+=num_threads) {
        ...
        for (int j=0; j<num_threads && j+i<count; j++) {
            threads[j] = thread(read_thread, filenames[i+j], result+i+j);
        }
        for (int j=0; j<num_threads && j+i<count; j++) {
            threads[j].join();
        }
